...
final class NetworkMonitor {
    ...
    func liveMarketDataPublisher(visibleIDs: [String], interval: TimeInterval = 15) -> AnyPublisher<(allCoins: [MarketCoin], watchlistCoins: [MarketCoin]), Never> {
        Timer.publish(every: interval, on: .main, in: .common)
            ...
            .flatMap { _ in
                Future { promise in
                    Task {
                        if let result = try? await CryptoAPIService.shared.fetchAllAndWatchlist(visibleIDs: visibleIDs) {
    ...
    /// Live-updating publisher for a single coin’s spot price on a timer.
    func liveSpotPricePublisher(for coin: String, interval: TimeInterval = 5) -> AnyPublisher<Double, Never> {
        Timer.publish(every: interval, on: .main, in: .common)
            .autoconnect()
            .flatMap { _ in
                Future { promise in
                    Task {
                        let price = (try? await CryptoAPIService.shared.fetchSpotPrice(coin: coin)) ?? 0
